...
to write-data-to-csv
  ;; Open the file to append data (if it doesn't exist, it will create a new one)
  if not file-exists? "simulation_results.csv" [
    file-open "simulation_results.csv"
    file-print "tick,average_commuting_time,average_taxi_occupancy,average_bus_occupancy"
    file-close
  ]

  ;; Open the file in append mode to add data for the current tick
  file-open "simulation_results.csv"
  file-print (word ticks "," analyze-citizen "," analyze-taxi)
  file-close
end
